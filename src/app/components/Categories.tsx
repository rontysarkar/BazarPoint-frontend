    ...
    return (
        <section className='max-w-7xl mx-auto px-4 py-4'>
            ...
            <div className='grid sm:grid-cols-3 lg:grid-cols-6 gap-3  '>
                {categories.slice(1,).map((cate, idx) => (
                    <div key={idx} className=' border border-slate-200 bg-white hover:shadow rounded-xl p-3 '>
                        <Link href={`/accounts?category=${cate.slug}`} className='flex  justify-center items-center gap-4'>
                            <div >
                                <Avatar className='w-12 h-12'>
                                    <AvatarImage className='object-cover' src={cate.logo} />
                                </Avatar>
                            </div>
